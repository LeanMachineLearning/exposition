...
| Statement anatomy (P4) | Each declaration's binder telescope split into its types, the other objects it is about with the typeclass structure on each, its hypotheses, and its claim — or, for a definition, its result type and its body: a structure's fields with their own docstrings, or a definition's value. One line per object by default, every constant's signature and docstring on hover, and an expanded view with a sentence per assumption. It replaced the pretty-printed signature block on the card, whose hovers it reproduces, and the source text folds under it. The first piece of layer 2 shown as content rather than listed. |
