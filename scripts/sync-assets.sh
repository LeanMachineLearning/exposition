...
# A full `lake build && exposition build-site` is only needed once the Lean-side page structure
# changes.
set -euo pipefail

if [[ $# -lt 1 ]]; then
  echo "Usage: $0 SITE_OUT_DIR" >&2
  echo "  SITE_OUT_DIR is the --output directory given to 'exposition build-site'." >&2
  exit 2
fi

repo_root="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
assets="$repo_root/LMLExposition/Website/assets"
out="$1"
...
if [[ ! -d "$dest" ]]; then
  ...
  for candidate in "$out/-verso-data" "$out"; do
    if [[ -d "$candidate" && -f "$candidate/exposition.css" ]]; then
      dest="$candidate"
      break
    fi
  done
...
if [[ ! -d "$dest" ]]; then
  echo "No built site assets at $dest (expected an 'exposition build-site' --output directory)." >&2
  exit 1
fi

copied=0
for f in exposition.css graph.js toc.js d3.v7.min.js; do
