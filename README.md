...
declaration gets a card with its docstring, its source-first statement, collapsible `Uses` /
`Used by`, and a dependency graph laid out in rows by depth. There is deliberately no whole-project
graph: one picture of every declaration is unreadable at any zoom and answers no question a reader
has.
